...
class _UnslothSFTTrainer(Trainer):
                ...
                if isinstance(eval_dataset, dict):
                    eval_dataset = {
                        key: self._prepare_dataset(
                            dataset, processing_class, args, packing, formatting_func, key
                        )
    ...
    def _prepare_dataset(
        ...
        if max_seq_length == 0:
            max_seq_length = getattr(args, "max_seq_length", 0)
        if max_seq_length == 0:
        ...
        if do_tokenize:
            ...
            if use_desc:
                map_kwargs["desc"] = f'Unsloth: Tokenizing ["{dataset_text_field}"]'
