...
class _UnslothDPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if (
            args.loss_type
            in ["hinge", "ipo", "bco_pair", "sppo_hard", "nca_pair", "apo_zero", "apo_down"]
    ...
    def concatenated_inputs(
        ...
        output["completion_input_ids"] = torch.cat(
            (
                pad_to_length(
                    batch["chosen_input_ids"], max_completion_length, pad_value=padding_value
                ),
                pad_to_length(
                    batch["rejected_input_ids"], max_completion_length, pad_value=padding_value
                ),
            ),
        )
    ...
    def dpo_loss(
        ...
        if self.f_divergence_type == FDivergenceType.ALPHA_DIVERGENCE.value:
            ...
            if (
                self.f_divergence_params
                and FDivergenceConstants.ALPHA_DIVERGENCE_COEF_KEY in self.f_divergence_params
            ):
                alpha_coef = float(
        ...
        rejected_rewards = (
            self.beta * (rejected_logps.to(device) - ref_rejected_logps.to(device)).detach()
        )
