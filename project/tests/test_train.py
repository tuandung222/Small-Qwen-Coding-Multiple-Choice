...
def mock_imports():
    ...
    with patch("src.train.AutoModelForCausalLM") as mock_model_class, \
         patch("src.train.AutoTokenizer") as mock_tokenizer_class, \
         patch("src.train.LoraConfig") as mock_lora_config, \
         patch("src.train.get_peft_model") as mock_get_peft_model, \
         patch("src.train.Trainer") as mock_trainer_class, \
         patch("src.train.TrainingArguments") as mock_training_args_class, \
         patch("src.train.load_from_disk") as mock_load_from_disk:
...
def test_setup_wandb_logging(mock_wandb, mock_wandb_logger):
    ...
    with patch("src.train.WandBLogger", return_value=mock_wandb_logger), \
         patch("src.train.WandBConfig") as mock_wandb_config:
        # Setup mock config
...
def test_setup_model_and_trainer(mock_wandb, mock_trainer, mock_model_handler, mock_wandb_logger):
    ...
    with patch("src.train.setup_wandb_logging", return_value=mock_wandb_logger), \
         patch("src.train.QwenTrainer", return_value=mock_trainer), \
         patch("src.train.ModelHandler", return_value=mock_model_handler), \
         patch("src.train.LoraConfig") as mock_lora_config, \
         patch("src.train.get_peft_model") as mock_get_peft_model, \
         patch("src.train.AutoModelForCausalLM.from_pretrained") as mock_from_pretrained, \
         patch("src.train.AutoTokenizer.from_pretrained") as mock_tokenizer_from_pretrained:
...
def test_train_basic_functionality(mock_wandb, mock_trainer, mock_model_handler, mock_wandb_logger):
    ...
    with patch("src.train.setup_model_and_trainer", return_value=(mock_model_handler, mock_trainer)), \
         patch("src.train.setup_wandb_logging", return_value=mock_wandb_logger), \
         patch("src.train.load_from_disk") as mock_load_from_disk:
        # Create test dataset
        train_dataset = create_mock_dataset()
...
def test_train_response_only_mode(mock_wandb, mock_trainer, mock_model_handler, mock_wandb_logger):
    ...
    with patch("src.train.setup_model_and_trainer", return_value=(mock_model_handler, mock_trainer)), \
         patch("src.train.setup_wandb_logging", return_value=mock_wandb_logger), \
         patch("src.train.load_from_disk") as mock_load_from_disk, \
         patch("unsloth.chat_templates.train_on_responses_only") as mock_train_on_responses:
        # Create test dataset
        train_dataset = create_mock_dataset()
...
def test_train_validation_strategies(mock_wandb, mock_trainer, mock_model_handler, mock_wandb_logger):
    ...
    with patch("src.train.setup_model_and_trainer", return_value=(mock_model_handler, mock_trainer)), \
        ...
        # Setup mock dataset loading
        mock_load_from_disk.side_effect = lambda path: train_dataset if "train_dataset" in path else val_dataset
...
def test_train_checkpoint_saving(mock_wandb, mock_trainer, mock_model_handler, mock_wandb_logger):
    ...
    with patch("src.train.setup_model_and_trainer", return_value=(mock_model_handler, mock_trainer)), \
         patch("src.train.setup_wandb_logging", return_value=mock_wandb_logger), \
         patch("src.train.load_from_disk") as mock_load_from_disk, \
         patch("src.train.save_checkpoint") as mock_save_checkpoint:
        # Create test dataset
        train_dataset = create_mock_dataset()
        ...
        for strategy in strategies:
            ...
            if strategy == "best":
                mock_save_checkpoint.assert_called_with(
                    mock_trainer,
                    os.path.join(TEST_DATA_DIR, "checkpoints", "best"),
                    "best"
                )
            elif strategy == "last":
                mock_save_checkpoint.assert_called_with(
                    mock_trainer,
                    os.path.join(TEST_DATA_DIR, "checkpoints", "last"),
                    "last"
                )
            elif strategy == "all":
