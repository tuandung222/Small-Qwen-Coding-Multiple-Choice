...
class _UnslothXPOTrainer(OnlineDPOTrainer):
    ...
    def _compute_judge(self, model_data, ref_data, context_length):
        ...
        if is_conversational({"prompt": prompts[0]}):
            ...
            model_data_completions = [
                template.render(messages=completion) for completion in model_data_completions
            ]
            ...
            ref_data_completions = [
                template.render(messages=completion) for completion in ref_data_completions
            ]
        ...
        return torch.tensor(
            [rank == 0 for rank in ranks_of_first_completion], device=model_data["input_ids"].device
        )
    ...
    def _log_statistics(
        ...
        model_eos = (
            model_data["input_ids"][:, context_length:] == self.processing_class.eos_token_id
        ).any(dim=1)
        ref_eos = (
            ref_data["input_ids"][:, context_length:] == self.processing_class.eos_token_id
        ).any(dim=1)
