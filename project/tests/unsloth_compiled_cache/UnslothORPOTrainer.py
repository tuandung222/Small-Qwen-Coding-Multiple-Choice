...
class _UnslothORPOTrainer(Trainer):
    ...
    def get_batch_loss_metrics(
        ...
        metrics[f"{prefix}log_odds_ratio"] = self.accelerator.gather_for_metrics(
            log_odds_ratio
        ).mean()
        metrics[f"{prefix}log_odds_chosen"] = self.accelerator.gather_for_metrics(
            log_odds_chosen
        ).mean()
        if is_torch_xla_available():
