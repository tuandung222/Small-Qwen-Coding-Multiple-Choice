...
def mock_hf_api():
    ...
    with patch("huggingface_hub.HfApi") as mock_api:
        ...
        mock_api.return_value.list_repo_commits.return_value = [
            {"id": "mock_commit_id", "title": "Initial commit", "date": "2024-03-29T00:00:00Z"}
        ]
...
def mock_hf_hub():
    ...
    with patch("huggingface_hub.hf_hub_download") as mock_download, patch(
        "huggingface_hub.snapshot_download"
    ) as mock_snapshot, patch("huggingface_hub.login") as mock_login, patch(
...
def test_model_initialization_with_error_handling(mock_unsloth, mock_env_vars):
    ...
    with pytest.raises(HfHubHTTPError):
        QwenModelHandler(model_name="error/model", model_source=ModelSource.UNSLOTH)

    # Test fallback to HuggingFace
    with patch("transformers.AutoModelForCausalLM.from_pretrained") as mock_model, patch(
...
def test_model_generation_error_handling(mock_unsloth, mock_env_vars):
    ...
    response = model_handler.generate_with_streaming(
        prompt="Test prompt", temperature=0.0, stream=False
    )
...
def test_wandb_integration(mock_wandb, mock_env_vars):
    ...
    logger = setup_wandb_logging(project_name="test_project", run_name="test_run")
...
def test_inference_workflow(mock_unsloth, mock_env_vars):
    ...
    # Test generation
    prompt = "What is 2+2?"
    response = model_handler.generate_with_streaming(prompt=prompt, temperature=0.0, stream=False)
    assert response is not None

    # Test streaming generation
    streamer = model_handler.generate_with_streaming(prompt=prompt, temperature=0.0, stream=True)
...
def test_error_handling(mock_env_vars):
    ...
    with pytest.raises(ValueError):
        QwenModelHandler(model_name="test", model_source="invalid")

    # Test invalid training paths
    with pytest.raises(ValueError, match="Train path does not exist"):
        train(model_name="test", train_path="nonexistent", output_dir=TEST_DATA_DIR)

    # Test invalid validation strategy
    with pytest.raises(ValueError, match="Invalid validation strategy"):
        train(model_name="test", train_path=TEST_DATA_DIR, validation_strategy="invalid")
