...
train_data = datasets.load_dataset("csv", data_files="/teamspace/studios/this_studio/workspace_1/data/raw/b6_train_data.csv", split='train')
...
train_data.save_to_disk('/teamspace/studios/this_studio/workspace_1/data/raw/parquet_format/train_data')
...
split_train.save_to_disk('/teamspace/studios/this_studio/workspace_1/data/raw/parquet_format/split_train')
split_val.save_to_disk('/teamspace/studios/this_studio/workspace_1/data/raw/parquet_format/split_val')
...
"""
    ...
    def _reasoning_with_label_template(self, question: str, choices: List[str], correct_answer: str) -> str:
        """
...
class MCTestingFramework:
    ...
    def __init__(
        ...
        if not api_key:
            raise ValueError("No OpenAI API key provided. Set it in ModelConfig or as environment variable.")
    ...
    def parse_response(self, response_text: str) -> tuple:
        ...
        try:
            ...
            if "answer:" in response_text.lower():
                # Find answer line
                answer_line = [line for line in response_text.split('\n') 
                              if line.lower().strip().startswith("answer:")][0]
    ...
    def evaluate_example(self, example: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Generate prediction
        predicted_answer, reasoning, response_time, raw_response, prompt = self.generate_answer(question, choices)
    ...
    def evaluate_batch(self, dataset, start_idx: int, end_idx: int) -> None:
        ...
        if self.test_config.show_progress:
            ...
            desc = f"Batch {start_idx//self.test_config.batch_size + 1}" if self.test_config.batch_size else f"Evaluating {self.model_config.name}"
    ...
    def evaluate_dataset(self, dataset) -> Dict[str, List]:
        ...
        if self.test_config.verbose:
            print(f"\nEvaluation complete.")
            print(f"Accuracy: {accuracy:.2%} ({sum(self.results['is_correct'])}/{len(self.results['is_correct'])})")
    ...
    def save_all_results(self) -> Dict[str, str]:
        ...
        output_dir = os.path.join(
            ...
            f"{self.model_config.name.replace('-', '_')}_{timestamp}"
        )
        ...
        if self.test_config.create_visualizations:
            ...
            for i, v in enumerate([metrics["correct_answers"], len(df) - metrics["correct_answers"]]):
...
def create_special_validation_dataset(
    ...
    if verbose and "num_choices" in special_val_dataset.features:
        choice_counts = pd.Series([ex["num_choices"] for ex in special_val_dataset]).value_counts().sort_index()
...
def create_special_training_dataset(
    ...
    if verbose and "num_choices" in special_train_dataset.features:
        choice_counts = pd.Series([ex["num_choices"] for ex in special_train_dataset]).value_counts().sort_index()
        print("\nDistribution of number of choices:")
        for choices, count in choice_counts.items():
...
def create_filtered_dataset(dataset, prefix="k", max_samples=50):
    ...
    filtered_examples = [
        ...
        if dataset[i]["task_id"].startswith(prefix)
    ]
...
class TeacherSynthesisFramework:
    ...
    def generate_synthetic_explanation(
        ...
        prompt = self.prompt_config._reasoning_with_label_template(question, choices, correct_answer)
    ...
    def process_dataset(self, dataset) -> Dict[str, Any]:
        ...
        if self.verbose:
            print(f"Generating synthetic explanations using {self.model_config.name} for {sample_size} examples...")
        ...
        if os.path.exists(progress_file):
            try:
                with open(progress_file, 'r') as f:
                    ...
                    if self.verbose:
                        print(f"Resuming from previous run. {len(processed_indices)} examples already processed.")
        ...
        for file in os.listdir(self.output_dir):
            if file.endswith(".yaml") and file.startswith(tuple(f"{i:06d}_" for i in range(sample_size))):
                try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.concurrent_requests) as executor:
            ...
            for future in tqdm(
                ...
                try:
                    ...
                    results.append({
                        "task_id": dataset[idx]["task_id"] if "task_id" in dataset[idx] else str(idx),
                        "error": str(e),
                        "is_correct": False
                    })
                    processed_indices.add(idx)
                ...
                if completed % save_interval == 0 or (current_time - last_save_time) > time_interval:
                    ...
                    if self.verbose and completed % save_interval == 0:
                        print(f"Progress saved after {completed}/{len(indices_to_process)} examples")
        ...
        if self.verbose:
            ...
            preservation_rate = self.metrics["correct_answer_preserved"] / self.metrics["total_examples"]
    ...
    def _calculate_metrics(self, results):
        ...
        self.metrics["successful_generations"] = sum(1 for r in results if "yaml_response" in r and r["yaml_response"])
        self.metrics["correct_answer_preserved"] = sum(1 for r in results if r.get("is_correct", False))
        self.metrics["failed_generations"] = self.metrics["total_examples"] - self.metrics["successful_generations"]
        self.metrics["reused_existing_files"] = sum(1 for r in results if r.get("reused_existing", False))
...
def run_synthesis(
    ...
    output_dir = '/teamspace/studios/this_studio/workspace_1/full_synthesis_results/gpt_4o_20250329_151040'
    # Set up model config
    model_config = ModelConfig(
        name=model_name,
...
directory = '/teamspace/studios/this_studio/workspace_1/full_synthesis_results/gpt_4o_20250329_151040'
...
class QwenTrainer:
    ...
    def __init__(self, model, tokenizer, prompt_creator=None, lora_config=None, hub_token=None, hub_model_id=None):
        """
    ...
    def train(self, dataset, prompt_type=None, output_dir="./model_output", 
              per_device_train_batch_size=4, gradient_accumulation_steps=4, 
              warmup_steps=100, num_train_epochs=3, max_steps=None, 
              learning_rate=2e-4, train_on_inputs=True, packing=False,
              logging_steps=10, save_steps=100, verbose=True,
              push_to_hub=False):
...
# %%
def create_default_dataset(path=None, parquet_file=None, task_ids=None, limit=None, output_dir=None, verbose=False):
    """
...
train_path = "/teamspace/studios/this_studio/workspace_1/data/raw/parquet_format/dataset_ready_for_training"
...
print(f"Python Example - Predicted: {result['predicted_answer']}, Correct: {result.get('is_correct', 'Unknown')}")
print("Reasoning:")
try:
...
test_dataset = datasets.load_dataset("csv", data_files="/teamspace/studios/this_studio/workspace_1/data/raw/b6_test_data.csv")
...
def generate_answer_csv(test_dataset, model_handler, prompt_creator):
    ...
    for batch_start in tqdm(range(0, num_examples, batch_size), desc="Processing batches"):
        ...
        for example in batch:
            ...
            prompt = prompt_creator.create_inference_prompt(example['question'], eval(example['choices']))
        ...
        for i, (task_id, output) in enumerate(zip(batch_task_ids, batch_outputs)):
            ...
            if answer_match:
                ...
                for letter in ['A', 'B', 'C', 'D']:
                    if f"({letter})" in output or f"{letter}." in output or f"answer {letter}" in output.lower():
