...
class ModelCompiler:
    ...
    def _compile_model(self) -> None:
        ...
        logger.info(
            f"Compiling model with mode={self.compile_mode}, backend={self.compile_backend}..."
        )
        ...
        try:
            ...
            self.compiled_model = torch.compile(self.original_model, **compile_kwargs)
    ...
    def optimize_attention(self) -> None:
        ...
        if not hasattr(self.original_model, "config"):
            logger.warning(
                "Model doesn't have a config attribute. Skipping attention optimizations."
            )
            return
