...
def parse_args() -> CompleteConfig:
    ...
    optimizer_group.add_argument(
        ...
        type=str,
        default="adamw",
        choices=["adamw", "torch_adamw"],
        help="Optimizer type",
    )
    optimizer_group.add_argument("--learning_rate", type=float, default=2e-5, help="Learning rate")
    optimizer_group.add_argument("--weight_decay", type=float, default=0.01, help="Weight decay")
    ...
    scheduler_group.add_argument("--warmup_ratio", type=float, default=0.1, help="Warmup ratio")
    ...
    training_group.add_argument(
        "--per_device_train_batch_size", type=int, default=4, help="Per device training batch size"
    )
    training_group.add_argument(
        "--gradient_accumulation_steps", type=int, default=4, help="Gradient accumulation steps"
    )
    training_group.add_argument("--logging_steps", type=int, default=10, help="Logging steps")
    training_group.add_argument("--save_steps", type=int, default=100, help="Save steps")
    training_group.add_argument(
        "--save_strategy",
        type=str,
        default="steps",
    ...
    validation_group.add_argument(
        "--per_device_eval_batch_size", type=int, default=4, help="Per device evaluation batch size"
    )
    validation_group.add_argument("--eval_steps", type=int, default=100, help="Evaluation steps")
    validation_group.add_argument(
        "--evaluation_strategy",
        type=str,
        default="steps",
    ...
    validation_group.add_argument(
        "--load_best_model_at_end", action="store_true", help="Load best model at end of training"
    )
    validation_group.add_argument(
        "--metric_for_best_model", type=str, default="accuracy", help="Metric for best model"
    )
    validation_group.add_argument(
        "--greater_is_better", action="store_true", help="Greater is better for metric"
    )

    # Dataset arguments
    dataset_group = parser.add_argument_group("Dataset")
    dataset_group.add_argument(
        "--dataset_id", type=str, required=True, help="Dataset ID from HuggingFace Hub"
    )
    dataset_group.add_argument(
    ...
    dataset_group.add_argument(
        "--response_only", action="store_true", help="Use response-only training"
    )

    # WandB arguments
    wandb_group = parser.add_argument_group("Weights & Biases")
    wandb_group.add_argument(
        "--wandb_enabled", action="store_true", help="Enable Weights & Biases logging"
    )
    wandb_group.add_argument(
        "--wandb_project",
        type=str,
    ...
    wandb_group.add_argument("--wandb_tags", type=str, nargs="+", help="Weights & Biases tags")

    # Environment arguments
    env_group = parser.add_argument_group("Environment")
    env_group.add_argument("--seed", type=int, default=42, help="Random seed")
    env_group.add_argument("--hf_token", type=str, required=True, help="HuggingFace token")
    env_group.add_argument("--output_dir", type=str, default="./outputs", help="Output directory")
